...
- **The `GOPROXY` chain is `file://` first, `proxy.golang.org` second, `direct` last — and the same chain is used in `ci-go`, `ci-cli` and `publish`.** Go falls through the list per request and only on a 404/410, and the file layout answers every request Go makes about the SDK module (`@v/list`, `.info`, `.mod`, `.zip`; a versionless `@latest` resolves from that `@v/list`). So `proxy.golang.org` is never asked about the unreleased version and cannot negative-cache it; it only ever serves third-party modules. In `cmd/go` terms: `modfetch.TryProxies` moves to the next entry only on a not-exist error (a 200 ends the request), `proxyRepo.Latest` falls back to the same proxy's `@v/list`, and retraction checks run only inside version queries — never in a pinned `tidy` or `build`. Verified with both slots logged at a version that existed nowhere but the file layout: slot 1 served exactly three SDK requests (`.info`, `.mod`, `.zip`), slot 2 saw 77 third-party requests and zero for `github.com/t-0-network/*` — across `tidy`, `build`, `build -mod=readonly`, `@latest`, `@upgrade` and a versionless `go get`.
...
- **Don't drop `proxy.golang.org` from the chain.** With `file://…,direct` every third-party module is fetched from its upstream git host, so the template build works only while every one of those repositories still exists under its module path. A repository that moves or goes private fails every template build — CI, release and publish alike — with `could not read Username for 'https://github.com'` from anonymous git, and nothing in this repo can fix that. `proxy.golang.org` serves pinned versions from an immutable cache regardless of upstream. The same applies to the `sumtool` invocation: no `GOPROXY=direct` prefix.
...
- **Don't let anything resolve the SDK version without the `file://` layout in front, and don't query `proxy.golang.org` for it yourself.** On the first request for a version the proxy fetches from GitHub. While the tag is not there — always on the release commit, and for a few seconds after `publish-go` pushes it — the 404 is cached for about 30 minutes, no retry loop outlasts it, and the version is unresolvable for everyone until the cache expires. The two ways to trigger that are a template build with the default `GOPROXY` on the release commit and a "warm the proxy" query right after the tag push; neither exists in the workflows, keep it that way. The only modules in this repo that pin the SDK are `go/starter/template` (covered by the layout) and `cross_test/go_helper` (local `replace`, never fetched).
