...
- **The `go mod download` before the chain is exported is a fail-fast gate.** With `GOPROXY=file://…` alone there is no fallthrough: if `sumtool` wrote nothing, or the wrong version string, the step dies with a local "no such file" error and zero network contact — rather than leaking a query for `go@vX.Y.Z` to `proxy.golang.org` minutes before `publish-go` creates that tag.
...
- **Don't query `proxy.golang.org` for a version whose tag was pushed seconds ago (v1.1.29).** The proxy fetches from GitHub on first request; if the tag has not replicated yet it caches the 404 for ~30 minutes, and no retry loop shorter than that recovers. The old "warm the proxy" step in `publish-go` did exactly this 78 ms after the tag push. It is gone; the only pre-tag consumer of the version is the `file://` layout, and the only post-tag build (`publish-go`) uses the same chain and is therefore served by the file layout too.
...
|---|---|---|---|
| `publish-go` | blacksmith | `go/sdkversion/version.go` matches tag | Creates + pushes the three Go module tags (`go/vX.Y.Z`, `go/starter/vX.Y.Z`, `go/starter/template/vX.Y.Z`). Tags are created here (not in `release.yaml`) so they exist before anyone can query the proxy for them — otherwise the proxy negative-caches a 404. It does **not** warm `proxy.golang.org` itself: that query, made seconds after the tag push, is what broke v1.1.29 (see "Things not to do here" above); the proxy indexes the tag on its own once GitHub has replicated it. Then `go build -mod=readonly ./...` in `go/starter/template` with the same `file://…,proxy.golang.org,direct` chain, verifying that the `go.sum` entry `release.yaml` precomputed matches a fresh `sumtool` pack of the tagged tree. `-mod=readonly` fails on a missing line as well as a wrong hash. This job pushes tags, never commits. No artifact upload — Go modules are served from the git tag itself. |
