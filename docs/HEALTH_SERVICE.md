...
**Node is the exception because npm gives it no other option.** The only `grpc.health.v1` build for protobuf-es lives on the Buf Schema Registry, and reaching it needs `@buf:registry=...` in the *installing* project's `.npmrc` — configuration a published package cannot hand to its consumers, so depending on it 404'd every downstream install. Regeneration instructions are in the file's header.

**This is why the SDKs serve `Check` and not `List`.** `List` is a recent addition to the health protocol and the packages the other four ecosystems depend on do not ship it — `Grpc.HealthCheck` has no `HealthList*` types at all, and the BSR Python build is pinned to a 2023 `health.proto`. Serving `List` would mean generating our own copy into those artifacts, which is the thing this design exists to avoid. `Check` has been in `health.proto` since 2015 and answers the only question the probe asks.
